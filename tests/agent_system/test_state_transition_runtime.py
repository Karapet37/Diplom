...
def test_chat_runtime_persists_current_context_and_transition_log(tmp_path, monkeypatch) -> None:
    ...
    assert current_context['task_procedure']['procedure_family']
    assert current_context['task_procedure']['response_form']
    assert current_context['task_procedure']['content_sources']
    ...
    assert transition_rows[-1]['task_procedure']['procedure_family']
    ...
    assert transition_rows[-1]['selected_response_mode']['response_form']
    ...
    assert payload['state_transition']['task_procedure']['procedure_family']
    assert payload['state_transition']['task_procedure']['success_criteria']
    ...
    assert payload['state_transition']['response_plan']['response_form']
